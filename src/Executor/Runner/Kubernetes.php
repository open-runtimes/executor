...
class Kubernetes extends Adapter
{
    ...
    private function init(array $networks): void
    {
        ...
        /**
         * Create and store Kubernetes networks (NetworkPolicies) used for communication between executor and runtimes
         */
        ...
        if (System::getEnv('OPR_EXECUTOR_IMAGE_PULL', 'enabled') === 'disabled') {
            ...
            foreach ($runtimeVersions as $runtimeVersion) {
                ...
                $runtimes = new Runtimes($runtimeVersion);
    ...
    public function getLogs(string $runtimeId, int $timeout, Response $response, Log $log): void
    {
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            if ($i === 9) {
                throw new \Exception('Runtime not ready. Pod not found.');
            }
        ...
        $logsProcess = \proc_open("tail -f -n +1 {$tmpLogging}/logs.txt", $descriptor, $pipes);
        ...
        foreach ($functionsToRemove as $container) {
            $jobsRuntimes[] = function () use ($container) {
                try {
                    ...
                    Console::success('Removed pod ' . $container->getName());
                } catch (\Throwable $th) {
                    Console::error('Failed to remove pod: ' . $container->getName());
                    Console::error($th);
                }
            };
        }
    ...
    private function createNetworks(array $networks): array
    {
        ...
        Console::info('Kubernetes runner does not require executor network connection.');
